...
int obj_add_file(const char *filename)
{
                              ...
                              &fm, sizeof (struct obj_file))) >= 0)
    {
        ...
        if (filename)
        {
            ...
            file(fi)->name = set_name(file(fi)->name, filename);
        }
    }
    return fi;
}
...
const char *obj_get_file_name(int fi)
{
    assert_file(fi);
    return file(fi)->name;
}
...
void obj_draw_mtrl(int fi, int mi)
{
    ...
    {
        if (GL_max_texture_image_units > 4 && obj_get_mtrl_map(fi, mi, OBJ_NS))
        {
            glActiveTexture(GL_TEXTURE4_ARB);
            obj_draw_prop(fi, mi, OBJ_NS);
        }
        if (GL_max_texture_image_units > 3 && obj_get_mtrl_map(fi, mi, OBJ_KS))
        {
            glActiveTexture(GL_TEXTURE3_ARB);
            obj_draw_prop(fi, mi, OBJ_KS);
        }
        if (GL_max_texture_image_units > 2 && obj_get_mtrl_map(fi, mi, OBJ_KE))
        {
            glActiveTexture(GL_TEXTURE2_ARB);
            obj_draw_prop(fi, mi, OBJ_KE);
        }
...
void obj_reset_all(void)
{
    int fi;

    /* Release all storage and cache. Revert to startup state. */
