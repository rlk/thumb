...
int uni::patch::visible(app::frustum_v& frusta, const double *hp)
{
    int d;
    ...
    for (app::frustum_i i = frusta.begin(); i != frusta.end(); ++i)
        if ((d = (*i)->test_shell(n0, n1, n2, hp, r0, r1)) >= 0)
            return 0;
                             ...
                             double bias, int d, GLsizei& count)
{
    ...
    if ((d > 0) || (d = visible(frusta, hp)) >= 0)
    {
        ...
        if (value(p) * bias > 1.0)  // HACK
        {
            ...
            {
                context c(ctx, 0);
                C[0] = C[0]->step(c, frusta, p, hp, bias, d, count);
            }
            if (C[1])
            {
                context c(ctx, 1);
                C[1] = C[1]->step(c, frusta, p, hp, bias, d, count);
            }
            if (C[2])
            {
                context c(ctx, 2);
                C[2] = C[2]->step(c, frusta, p, hp, bias, d, count);
            }
            if (C[3])
            {
                context c(ctx, 3);
                C[3] = C[3]->step(c, frusta, p, hp, bias, d, count);
            }
